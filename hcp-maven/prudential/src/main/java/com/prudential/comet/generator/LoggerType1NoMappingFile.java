...
public class LoggerType1NoMappingFile extends PrudentialBaseFiles {
	...
	public void initialize() { 
		if ( ! isInitialized ) {			
			...
			mInputDateFormat = new SimpleDateFormat("MMddyy'_'HHmmss");
	...
	protected String getCustomMetadata(File inSourceFile) {
		...
		try {
			...
			try {
				...
				EndDate = mOutputDateFormat.format(mInputDateFormat.parse(fileNameCore.substring(23, 36).trim()));
				CLSCallID = null;
				Session = null;
				Channel = fileNameCore.substring(37).trim();
				Server = null;
				AgentID = null;
